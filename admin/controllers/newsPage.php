...
if (isset($_POST['searchSubmit'])) {
    ...
    if (!empty($newsTitle))
        $where.= (!empty($where)) ? " AND title LIKE '%$newsTitle%'" : "title LIKE '%$newsTitle%'";
    $newsText = $_POST['text'];
    if (!empty($newsText))
        $where.= (!empty($where)) ? " AND `text` LIKE '%$newsText%'" : "`text` LIKE '%$newsText%'";
...
if (isset($_POST['newsSubmit'])) {
    ...
    if (isset($_POST['publication_date'])) {
        ...
        $query = "INSERT INTO news (title, lead, text, date, tag, publication_date, author_id, corrector_id) VALUES "
                . "('$newsTitle', '$newsLead', '$newsText', '$newsDate', '$newsTag', '$publication_date', $author_id, $author_id);";
    } else {
        $query = "INSERT INTO news (title, lead, text, date, tag, author_id) VALUES "
                . "('$newsTitle', '$newsLead', '$newsText', '$newsDate', '$newsTag', $author_id);";
    }
